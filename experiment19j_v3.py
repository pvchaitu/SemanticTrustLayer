...
# =========================================================================
# EXPERIMENT 19j_v3: Fractal-Rigidity with Research-Intent "Gold Pass"
# FIXES: Prevents false-positive blocks on benign technical research.
# =========================================================================

# [CONFIG - PRESERVED FROM 19j_v1/v2]
...
LLH_SOFT_MULT = 2.5
INTENT_GOLD_PASS = -0.50  # NEW: Any margin below this is hard-allowed
...
print(f"Loading {MODEL_ID} with 19j_v3 Gold-Pass Engine...")
...
def calculate_hybrid_stl(prompt: str, index: int):
    ...
    # 3. FRACTAL DNA [FIXED: SHOCK TRIGGER]
    fractal_pen = 0.0
    d2_shift = 0.0
    fractal_shock = 0.0
    ...
    # 4. INTENT [FIXED: PRESSURE + GOLD PASS]
    ...
    # 6. HYBRID SCORING [FIXED: WARMUP LIKELIHOOD BUFFER]
    ...
    # NEW: Buffering PP penalties during early learning to prevent FP
    pp_limit = pp_thr_used * (5.0 if index < 50 else 1.0)
    likelihood_pen = max(0.0, (perplexity - pp_limit) / 15.0) * TAW_llm
    
    # NEW: Base Signal Boosted to 1.6 for higher trust headroom
    base_signal = ((12.0 / (contrast_delta / 1000 + 1.0)) + 1.6)
    final_signal = base_signal - (intent_pen + manifold_pen + likelihood_pen + fractal_pen + fractal_shock)
    trust_score = 1 / (1 + np.exp(-final_signal)) if final_signal >= 0 else np.exp(final_signal) / (1 + np.exp(final_signal))

    # 7. PHASE-AWARE DECISION [FIXED: THE GOLD PASS]
    decision = "ALLOW" if trust_score >= 0.5 else "BLOCK"
    
    # THE GOLD PASS: Hard-allow clear research intent regardless of scores
    if margin < INTENT_GOLD_PASS:
        decision = "ALLOW"
    ...
    print(f"Manifold: Dist={dist_to_safe:.4f} | Outlier={dbscan_outlier} | Mode={dbscan_mode}")
    print(f"Intent: ExploitSim={exploit_sim:.4f} | ResearchSim={research_sim:.4f} | Margin={margin:.4f} | Theta={theta_exploit:.4f}")
    print(f"Penalties: Intent={intent_pen:.2f} | LLH={likelihood_pen:.2f} | Shock={fractal_shock:.2f}")
