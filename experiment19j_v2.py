...
# =========================================================================
# EXPERIMENT 19j_v2: Fractal-Augmented STL with Rigid Manifold Convergence
# FULL INTEGRATION: Structural Pressure + Fractal Shock Tightening
# =========================================================================

# [CONFIG - PRESERVED FROM 19j_v1]
...
# [MATURITY CRITERIA - PRESERVED FROM 19j_v1]
MIN_BASE_CSTA = 30
BENIGN_WINDOW = 80
...
print(f"Loading {MODEL_ID} with Full 19j_v2 Tightened Engine...")
...
def calculate_hybrid_stl(prompt: str, index: int):
    ...
    # 3. FRACTAL RIGIDITY & SHOCK DETECTION [FIXED TIGHTENING]
    fractal_pen = 0.0
    d2_shift = 0.0
    fractal_shock = 0.0
    if len(trusted_manifold) > 10:
        ...
        if current_d2 > 0:
            ...
            # TIGHTENING: Sudden structural changes trigger suspicion even in OBSERVE
    ...
    # 4. INTENT & STRUCTURAL PRESSURE [FIXED TIGHTENING]
    ...
    if index >= MANIFOLD_WARMUP_AT:
        ...
        # TIGHTENING: Outliers amplify intent suspicion (Structural Pressure)
        pressure_mult = 1.0 + (dist_to_safe * 15.0 if dbscan_outlier else 0.0)
        intent_pen *= pressure_mult
    ...
    total_penalty = intent_pen + manifold_pen + energy_pen + likelihood_pen + fractal_pen + fractal_shock
    ...
    # 7. PHASE-AWARE DECISION [FIXED TIGHTENING]
    decision = "ALLOW" if trust_score >= 0.5 else "BLOCK"
    if dbscan_mode == "OBSERVE" and index >= MANIFOLD_WARMUP_AT:
        # TIGHTENED OVERRIDE: Must be linguistically benign AND structurally stable
        is_structurally_sound = (not dbscan_outlier) and (fractal_shock < 2.0)
        
        if _is_likely_benign(margin, perplexity, energy_score, pp_thr_used) and is_structurally_sound:
            decision = "ALLOW"
        else:
            # If it's a structural outlier or shows intent, BLOCK immediately
            if dbscan_outlier or fractal_shock > 5.0 or margin > theta_exploit:
                decision = "BLOCK"
            else:
                decision = "REVIEW"
    ...
    if trusted_manifold: research_anchor = _normalize(np.mean(np.stack(trusted_manifold), axis=0))
    ...
    print(f"\n--- [19j_v2-TIGHTENED] Index {index} ---")
    print(f"Prompt: {prompt[:50]}...")
    print(f"Signals: PP={perplexity:.2f} | Energy={energy_score:.2f} | Contrast={contrast_delta:.0f}")
    print(f"Fractal: D2={current_d2:.4f} | Shift={d2_shift:.4f} | Shock={fractal_shock:.2f}")
    ...
    print(f"Penalties: Intent={intent_pen:.2f} | Shock={fractal_shock:.2f} | LLH={likelihood_pen:.2f}")
    print(f"RESULT: Trust={trust_score:.4f} | Decision=[{decision}]")
    
    return trust_score, decision

# [TEST SUITE - FULL 52 PROMPTS]
test_prompts = [
