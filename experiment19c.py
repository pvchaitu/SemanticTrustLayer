...
# =========================================================================
# 1. ARCHITECTURAL CALIBRATION & INDUSTRY STANDARDS (v10c - Experiment 19c)
# =========================================================================
# NOTE: This file refines Experiment-19/19b to fix Option-B intent gating:
#  - keep margin = exploit_sim - research_sim (directional)
#  - enforce non-negative theta (theta >= 0), preventing negative thresholds
#  - use a numerically-stable sigmoid
#  - preserve Exp-18 output ordering; add one extra INTENT line for research
# Unmodified comments/logic from Exp-18/19 are intentionally retained.
...
# Warm-up derived from benign test count; cap at 200 to preserve Exp-18 feel.
...
print(f"Initializing Multi-Anchor Geometric Trust (v10c - Experiment 19c)...")
...
theta_exploit = 0.0  # must be >= 0 to avoid invalid negative thresholds
...
# ---- Option B: dynamic warm-up aware update (theta >= 0) -------------------
# If we do not yet have enough benign exemplars to compute a robust
# percentile threshold, keep theta at 0.0 so intent can contribute mildly
# (only when exploit_sim > research_sim). When warm-up is reached, compute
# the percentile and clamp to >= 0 to avoid negative thresholds.
...
def _update_theta_exploit():
    ...
    if len(intent_benign_buffer) >= INTENT_BUFFER_WARMUP_DYNAMIC:
        ...
        theta_exploit = max(theta_exploit, 0.0)  # enforce non-negative gate
    else:
        theta_exploit = max(theta_exploit, 0.0)  # keep at non-negative fallback
...
def _maybe_update_anchors():
    ...
    if len(blocked_embeddings) >= 10:
        ...
        exploit_anchor = -research_anchor  # temporary fallback; intent gated by theta>=0

# ====================== Core Scoring (Exp-19c) ==============================
...
def calculate_hybrid_stl(prompt: str, index: int):
    ...
    if index < MANIFOLD_FROZEN_AT:
        ...
        if (research_anchor is not None) and (exploit_anchor is not None):
            # margin uses directional score (exploit - research)
    ...
    # Option B: margin + adaptive theta (theta >= 0) ------------------------
    margin = exploit_sim - research_sim  # positive only when exploit > research
    intent_penalty = 0.0
    ...
    # Add-on line (Exp-19c): minimal extra fields to aid future analysis
