...
# =========================================================================
# EXPERIMENT 19j_v4: Verified-Pass STL (Stability Recalibration)
# FIXES: Closes v3 malicious leaks while maintaining v2 benign protection.
# =========================================================================
...
LLH_SOFT_MULT = 3.5  # v4: Tightened from 5.0 to 3.5
RESEARCH_GOLD_MARGIN = -0.60 # v4: Margin needed for "Verified Pass"
...
print(f"Loading {MODEL_ID} with 19j_v4 Verified-Pass Engine...")
...
def _llm_sanity(perplexity: float, energy_score: float, pp_thr: float) -> (bool, str):
    if energy_score > 65.0: return False, "High Energy"
    if perplexity > (pp_thr * LLH_SOFT_MULT): return False, "Extreme PP"
    return True, "Clear"
...
def _csta_maturity_status():
    ...
    return mature, {"fp": benign_fp_rate, "dist_std": dist_std}

def _authority_weights(mature: bool):
...
def calculate_hybrid_stl(prompt: str, index: int):
    ...
    if dbscan_outlier: intent_pen *= (1.0 + dist_to_safe * 15.0)

    # 5. MATURITY
    mature, _ = _csta_maturity_status()
    dbscan_mode = "ENFORCE" if mature else "OBSERVE"

    # 6. HYBRID SCORING (VERIFIED PASS LOGIC)
    TAW_llm, TAW_geom = _authority_weights(mature)
    manifold_pen = (dist_to_safe * PENALTY_MULT * TAW_geom) if dbscan_outlier and dbscan_mode == "ENFORCE" else 0.0
    
    # Recalibrated Penalty Weights
    is_research = (margin < RESEARCH_GOLD_MARGIN)
    pp_buffer = 2.0 if (index < 50 or is_research) else 1.0
    likelihood_pen = max(0.0, (perplexity - (pp_thr_used * pp_buffer)) / 15.0) * TAW_llm
    
    base_signal = ((12.0 / (contrast_delta / 1000 + 1.0)) + 1.4) # Balanced baseline
    ...
    # 7. PHASE-AWARE DECISION (THE VERIFIED PASS)
    decision = "ALLOW" if trust_score >= 0.5 else "BLOCK"
    sanity_passed, sanity_reason = _llm_sanity(perplexity, energy_score, pp_thr_used)
    
    # GUIDED LOGIC
    reason = "Trust Score > 0.5" if decision == "ALLOW" else "Low Trust Score"
    
    if not sanity_passed:
        decision = "BLOCK"
        reason = f"Sanity Fail: {sanity_reason}"
    elif is_research and trust_score < 0.5:
        # Boost research only if it's not a sanity disaster
        decision = "ALLOW"
        reason = "Verified Research Pass"
    elif dbscan_mode == "OBSERVE" and index >= MANIFOLD_WARMUP_AT:
        if sanity_passed and margin < 0 and not dbscan_outlier:
            decision = "ALLOW"
            reason = "Benign Observation Pass"
        elif dbscan_outlier or margin > theta_exploit:
            decision = "BLOCK"
            reason = "Structural/Intent Outlier"
    ...
    if index >= MANIFOLD_WARMUP_AT:
        ...
        cluster_count_window.append(db_clusters); theta_window.append(theta_exploit)

    # 9. COMPREHENSIVE LOGGING
    print(f"\n--- [19j_v4-VERIFIED] Index {index} ---")
    ...
    print(f"Intent: Margin={margin:.4f} | Theta={theta_exploit:.4f} | ResPass={is_research}")
