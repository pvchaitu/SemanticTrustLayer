...
# =========================================================================
# EXPERIMENT 19j_v1: Fractal-Augmented STL with Rigid Manifold Convergence
# (Full 500+ Line Integration of 19i_v1 + Fractal DNA Intelligence)
# =========================================================================

# [CONFIG - PRESERVED FROM 19i_v1]
...
# -------------------------------------------------------------------------
# NEW: Fractal & Rigidity Parameters
# -------------------------------------------------------------------------
RIGIDITY_WINDOW = 40        # Window to observe Fractal Dimension stability
D2_VAR_EPS = 0.015          # Stability threshold for Fractal Dimension
FRACTAL_DISRUPT_MULT = 15.0 # Penalty for prompts violating fractal DNA
# -------------------------------------------------------------------------

print(f"Loading {MODEL_ID} with Fractal Rigidity Engine...")
...
# [NEW UTILITY: FRACTAL DIMENSION]
def _calculate_fractal_dimension(points: np.ndarray, r=0.1) -> float:
    """
    Computes Correlation Dimension (D2). 
    Measures the probability that two points are within distance 'r'.
    """
...
# [MATURITY STATUS - AUGMENTED WITH FRACTAL CHECK]
def _csta_maturity_status():
    ...
    # NEW: Fractal Stability
    d2_std = float(np.std(np.array(d2_history))) if len(d2_history) >= 10 else 999.0
    ...
    rigidity_ok = (d2_std <= D2_VAR_EPS) # Fractal convergence check

    mature = (csta_size >= MIN_BASE_CSTA and fp_ok and dist_ok and 
              theta_ok and cluster_ok and noise_ok and rigidity_ok)

    details = {
        "ok": {"fp": fp_ok, "dist": dist_ok, "theta": theta_ok, "cluster": cluster_ok, "noise": noise_ok, "rigidity": rigidity_ok},
        "metrics": {"fp": benign_fp_rate, "dist_std": dist_std, "theta_std": theta_std, "cluster_p": cluster_persist, "noise_r": noise_ratio_avg, "d2_std": d2_std}
    }
    return mature, details

def _authority_weights(mature: bool, maturity_details: dict):
    if mature: return 0.0, 1.0
    ev_ratio = float(np.mean([len(benign_outlier_window)/BENIGN_WINDOW, len(benign_dist_window)/DIST_WINDOW]))
    taw_geom = 0.6 * ev_ratio
    return 1.0 - taw_geom, taw_geom
...
def calculate_hybrid_stl(prompt: str, index: int):
    ...
    # 3. NEW: FRACTAL DIMENSION & RIGIDITY ANALYSIS
    fractal_pen = 0.0
    d2_shift = 0.0
    if len(trusted_manifold) > 10:
        ...
        if current_d2 > 0:
            ...
            # If manifold is in ENFORCE, sudden structural shifts incur penalty
            if dbscan_mode == "ENFORCE" and d2_shift > 0.15:
                fractal_pen = d2_shift * FRACTAL_DISRUPT_MULT
    ...
    if index >= MANIFOLD_WARMUP_AT:
        intent_pen = max(0.0, (margin - theta_exploit) * INTENT_SCALE_K)

    # 5. MATURITY & MODE SWITCH
    mature, maturity = _csta_maturity_status()
    prev_mode = dbscan_mode
    dbscan_mode = "ENFORCE" if mature else "OBSERVE"
    if prev_mode == "OBSERVE" and dbscan_mode == "ENFORCE":
        print(f"\n!!! [MODE-SWITCH] Manifold Mature at Index {index} !!!")
        print(f"Metrics: {maturity['metrics']}")

    # 6. HYBRID SCORING
    TAW_llm, TAW_geom = _authority_weights(mature, maturity)
    manifold_pen = (dist_to_safe * PENALTY_MULT * TAW_geom) if dbscan_outlier and dbscan_mode == "ENFORCE" else 0.0
    likelihood_pen = max(0.0, (perplexity - pp_thr_used) / 10.0) * TAW_llm
    energy_pen = (energy_score / ENERGY_DIVISOR) * TAW_llm
    
    # Combined Structural & Intent Penalty
    total_penalty = intent_pen + manifold_pen + energy_pen + likelihood_pen + fractal_pen
    ...
    # 7. PHASE-AWARE DECISION
    decision = "ALLOW" if trust_score >= 0.5 else "BLOCK"
    if dbscan_mode == "OBSERVE" and index >= MANIFOLD_WARMUP_AT:
        # Override: Don't hard block benign-looking prompts during learning
        if _is_likely_benign(margin, perplexity, energy_score, pp_thr_used):
            decision = "ALLOW"
        else:
            # Extreme OOD check for hard-blocking in OBSERVE
            if (perplexity > pp_thr_used * HARD_PP_MULT) or (energy_score > HARD_ENERGY):
                decision = "BLOCK"
            else:
                decision = "REVIEW"

    # 8. STATE UPDATES
    if decision == "ALLOW" and margin < 0:
        trusted_manifold.append(embedding)
    ...
    # Freeze exploit learning during OBSERVE to prevent semantic contamination
    if dbscan_mode == "ENFORCE" and decision == "BLOCK":
    ...
    if index >= MANIFOLD_WARMUP_AT:
        ...
        noise_ratio_window.append(_db_labels_noise / max(1, len(trusted_manifold)))
        theta_window.append(theta_exploit)

    # 9. COMPREHENSIVE LOGGING [PRESERVED FROM 19i_v1]
    print(f"\n--- [19j-FRACTAL] Index {index} ---")
    print(f"Prompt: {prompt[:50]}...")
    print(f"Signals: PP={perplexity:.2f} (Thr={pp_thr_used:.1f}{'*' if pp_is_dynamic else ''}) | Energy={energy_score:.2f} | Contrast={contrast_delta:.0f}")
    print(f"Fractal: D2={current_d2:.4f} | Shift={d2_shift:.4f} | Rigidity={'OK' if maturity['ok']['rigidity'] else 'STAB...'}")
    print(f"Manifold: Dist={dist_to_safe:.4f} | Outlier={dbscan_outlier} | Mode={dbscan_mode} | Clusters={db_clusters}")
    print(f"Intent: ExploitSim={exploit_sim:.4f} | ResearchSim={research_sim:.4f} | Margin={margin:.4f} | Theta={theta_exploit:.4f}")
    print(f"Penalties: Intent={intent_pen:.2f} | Geom={manifold_pen:.2f} | Fractal={fractal_pen:.2f} | LLH={likelihood_pen:.2f}")
    print(f"RESULT: Trust={trust_score:.4f} | Decision=[{decision}]")
